angular.module('app.controllers', [])
    ...
    .controller('mainCtrl',['$scope','$rootScope','$http','$state','addAttentionServices','attentionServices','$location',function($scope,$rootScope,$http,$state,addAttentionServices,attentionServices,$location){
       ...
       $rootScope.$on('$stateChangeSuccess',function(event, toState, toParams, fromState, fromParams){ 
	        ...
	        if(localStorage.getItem('token')){ 
	        		if(JSON.parse( localStorage.getItem('token'))){
	        			var guoqi_time=JSON.parse( localStorage.getItem('token')).guoqi_time;
	        			var login_time=JSON.parse( localStorage.getItem('token')).login_time; 
	        			if(new Date().getTime()>(guoqi_time+login_time)){ 
						   //如果超时。清理用户信息 
						      localStorage.removeItem('user');
                   			  localStorage.removeItem('userData');
                              localStorage.removeItem('token');
	        				   if(toState.name!='login'&&toState.name!='forget'){   
	        				   		$state.go('login' ); 
	        				   		location.reload();
	        				   	} 
	        			} 
	        	...
	        	// 如果没有登录
	        	 if(toState.name=='login'||toState.name=='forget'||toState.name=='app.downLoad') 
	        		 {  
	        		 	return;
	        		 }else{   
	       			$state.go('login'); 
                ...
                if(data.result === 0){
                    ...
                    loginServices.getTonken({
                        ...
                        token: $rootScope.userData.registration_id+'8e9b109eedc27959233242342342'
                    		...
                    		var obj_token={
                        token: 1,
                        guoqi_time:  86400000 ,//1天过期
                       	login_time:new Date().getTime()
                    }; 
                        data.result == 1 && localStorage.setItem('token',JSON.stringify(obj_token));
    ...
    .controller('actually',['$scope','$rootScope','actuallyService','$state',function($scope,$rootScope,actuallyService,$state){
      //console.log($rootScope);
        ...
        //排行模块
        	//如果登录了，判断访问权限
				var have_privilege=false;
				var result_privilege=JSON.parse(localStorage.getItem('userData')).privilege;
			  	//设置权限
			  	$cur_model='ranking';//toState.name.substring( toState.name.lastIndexOf('.')+1);
			    angular.forEach(result_privilege,function(data){ 
					//如果有权限
		        		if(data.privilege_content===$cur_model){
		        			  have_privilege=true;
		        		} 
		    		}); 
              if(!have_privilege){
    ...
    .controller('actuallyContent',['$scope','$rootScope','actuallyContestService','excel_Service','$stateParams','$state',function($scope,$rootScope,actuallyContestService,excel_Service,$stateParams,$state){
        	...
        	//如果登录了，判断访问权限
				...
				var result_privilege=JSON.parse(localStorage.getItem('userData')).privilege;
			  	//设置权限
			  	$cur_model='actually';//toState.name.substring( toState.name.lastIndexOf('.')+1);
			    angular.forEach(result_privilege,function(data){ 
					//如果有权限
		        		if(data.privilege_content===$cur_model){
		        			  have_privilege=true;
...
//                      val = target.innerHTML;
    ...
    }]).controller('rankingContent',['$scope','$rootScope','$stateParams','rankingServices','$state',function($scope,$rootScope,$stateParams,rankingServices,$state){
       			//如果登录了，判断访问权限
				var have_privilege=false; 
				var result_privilege=JSON.parse(localStorage.getItem('userData')).privilege;
			  	//设置权限
       ...
       $rootScope.var.isSSHead = 0;
        ...
        $rootScope.fnModule($stateParams,rankingServices,20,'rankingData');
				...
				var result_privilege=JSON.parse(localStorage.getItem('userData')).privilege;
			  	//设置权限
			  	$cur_model='getLand';//toState.name.substring( toState.name.lastIndexOf('.')+1);
			    angular.forEach(result_privilege,function(data){  
					//如果有权限
		        		if(data.privilege_content===$cur_model){
		        			  have_privilege=true;
    ...
    }]).controller('designContent',['$scope','$rootScope','designServices','$stateParams','$state',function($scope,$rootScope,designServices,$stateParams,$state){
        			//如果登录了，判断访问权限
				var have_privilege=false; 
				var result_privilege=JSON.parse(localStorage.getItem('userData')).privilege;
			  	//设置权限
			  	$cur_model='design';//toState.name.substring( toState.name.lastIndexOf('.')+1);
			    angular.forEach(result_privilege,function(data){ 
					//如果有权限
		        		if(data.privilege_content===$cur_model){
		        			  have_privilege=true;
    ...
    }]).controller('marketingContent',['$scope','$rootScope','marketingServices','$stateParams','$state',function($scope,$rootScope,marketingServices,$stateParams,$state){
       			//如果登录了，判断访问权限
				var have_privilege=false; 
				var result_privilege=JSON.parse(localStorage.getItem('userData')).privilege;
			  	//设置权限
			  	$cur_model='marketing';//toState.name.substring( toState.name.lastIndexOf('.')+1);
			    angular.forEach(result_privilege,function(data){ 
					//如果有权限
		        		if(data.privilege_content===$cur_model){
		        			  have_privilege=true;
    ...
    }]).controller('superSearch',['$scope','$rootScope','superSearchServices','excel_Service','getLandServices','$state',function($scope,$rootScope,superSearchServices,excel_Service,getLandServices,$state){
       			//如果登录了，判断访问权限
				var have_privilege=false; 
				var result_privilege=JSON.parse(localStorage.getItem('userData')).privilege;
			  	//设置权限
			  	$cur_model='superSearch';//toState.name.substring( toState.name.lastIndexOf('.')+1);
			    angular.forEach(result_privilege,function(data){ 
					//如果有权限
		        		if(data.privilege_content===$cur_model){
		        			  have_privilege=true;
